# 3: Diffusion in two dimensional comb
*07-Oct-21* 
...
### Mimicing the Dirac $\delta$-function, with $A(y)$

The function can by replicated by $A(y) = \frac{1}{\sqrt{2\pi}\sigma}\exp(-y^2/[2\sigma^2])$ in the limit $\sigma\to 0$, for different values for $\sigma=[1, 0.5, 0.01]$
...
class Brownian_motion_Langevin:
    ...
    def solve2d(self):
      ...
      for t in range(len(self.times)-1):
        ...
        r[0][t+1] = r[0][t] + 0.5 * (A(r[1][t]) + A(r[1][t+1]))* dwy
...
ax.plot(x, (lambda x: factor*x**0.5)(x), linewidth=0.7, alpha=0.9, label=r"Theory", color="#2C2891")
...
plt.xlabel(r"time, $t$")
plt.title(r"MSD; $\langle x^2(t)\rangle=2 \frac{\mathcal{D}_y}{2\sqrt{\mathcal{D}_x}}t^{\frac{1}{2}}$", fontsize=9, pad=10)
...
ax.plot(x, (lambda x: 2*Dy*x**1)(x), linewidth=0.7, alpha=0.7, label=r"Theory", color="#2C2891")
